    ...
    int maxTurns(vector<int> cards) {
        ...
        while (siz>0) {
            ...
            for (int i=0; i<cards.size(); i++) {
                ...
                int cn=0;
                int crd = cards[i];
                if (i-1>-1 && cards[i-1]+1==crd) {
                    cn++;
                }
                if (i+1<cards.size() && cards[i+1]-1 == crd) {
                    cn++;
                }
                if (cn<m) {
                    m=cn;
            ...
            if (id-1>-1 && cards[id-1]+1==cards[id]) {
                siz--;
                cards[id-1]=-1;
            }
            if (id+1<cards.size() && cards[id+1]-1 == cards[id]) {
                siz--;
                cards[id+1]=-1;
            }
            cards[id]=-1; siz--;
...
namespace otocoder {
    ...
    int run_test_case(int casenum) {
        ...
        case 0: {
            ...
            int received_ = SRMCards().maxTurns(vector<int>(cards, cards + (sizeof cards / sizeof cards[0])));
    ...
    bool run_full_test() {
        ...
        for (int i=0; i<testCase; i++) {
            ...
            for(int j=0; j<N; j++) {
                std::cin>>cards_sin; std::cin.ignore(10, '\n');
                cards.push_back(cards_sin);
            }
            std::cin>>expected_; std::cin.ignore(10, '\n');
            clock_t start_           = clock();
            int received_ = SRMCards().maxTurns(cards);
        ...
        if (testCase == 0) {
            ...
            std::ofstream ofs("SRMCards.st", std::ios_base::app);
