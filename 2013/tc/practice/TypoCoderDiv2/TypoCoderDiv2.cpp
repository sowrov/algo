...
public:
    int count(vector<int> rating) {
		...
		for (int i=0; i<rating.size(); i++) {
			if (rating[i]>=1200 && r<1200) {
				out++;
			} else if (rating[i]<1200 && r>=1200) {
				out++;
			}
			r = rating[i];
		}
...
namespace otocoder {
    ...
    int run_test_case(int casenum) {
        ...
        case 0: {
            ...
            int received_ = TypoCoderDiv2().count(vector<int>(rating, rating + (sizeof rating / sizeof rating[0])));
    ...
    //Command line example: TypoCoderDiv2.exe -2 <TypoCoderDiv2IO.txt
    bool run_full_test() {
        ...
        for (int i=0; i<testCase; i++) {
            ...
            for(int j=0; j<N; j++) {
                std::cin>>rating_sin; std::cin.ignore(10, '\n');
                rating.push_back(rating_sin);
            }
            std::cin>>expected_; std::cin.ignore(10, '\n');
            clock_t start_           = clock();
            int received_ = TypoCoderDiv2().count(rating);
        ...
        if (testCase == 0) {
            ...
            std::ofstream ofs("TypoCoderDiv2.st", std::ios_base::app);
