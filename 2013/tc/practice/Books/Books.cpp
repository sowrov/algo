...
public:
    int sortMoves(vector<string> titles) {
        ...
        sort (vtitle.begin(), vtitle.end(), comp);
        int maxV = 0;
        for (int i=0; i<vtitle.size(); i++) {
            for (int j=i-1; j>=0; j--) {
                if (vtitle[j].index < vtitle[i].index) {
...
namespace otocoder {
    ...
    //Command line example: Books.exe -2 <BooksIO.txt
    bool run_full_test() {
