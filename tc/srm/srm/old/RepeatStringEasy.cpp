...
public:
    int maximalLength( string s ) {
		...
		for (int i = 0; i < s.size() - 1; i++) {
			maxv = max(maxv, lcs(i, s));
		}
		return 2*maxv;
    }

	int lcs(int p, string s) {
		Zclr(memo);
		int maxv = -1;
		for (int i = 0; i <= p; i++) {
			for (int k=0, j = p + 1; j < s.size(); k++, j++) {
				if (s[i] == s[j]) {
					if (i == 0 || k == 0) {
						memo[i][k] = 1;
					} else {
					...
					memo[i][k] = max(top, left);
				}
				maxv = max(maxv, memo[i][k]);
			}
		}
		return maxv;
	}
...
namespace moj_harness {
    ...
    int run_test_case(int casenum) {
         ...
         case 0: {
                   ...
                   int received__            = RepeatStringEasy().maximalLength(s);
